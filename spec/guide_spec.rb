...
describe Guide do
  ...
  let(:blank_guide) { described_class.new(new_file) }
  ...
  describe '#launch!' do
    it 'outputs a introductory message' do
      ...
      expect { subject.launch! }.to output(/Finder/).to_stdout
    end
  end
  ...
  describe 'performing actions' do
    ...
    xcontext 'with list action' do
      ...
      it 'sorts by cuisine when asked' do
        ...
        expect(cuisines).to eq(cuisine.sort)
      end
    end

    xcontext 'with find action' do
      ...
      it 'finds restaurants with matching name keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq('-' * 60)
        expect(lines[12]).to include('Cafe Nero')
        expect(lines[13]).to eq('-' * 60)
      end
      ...
      it 'finds restaurants with matching cuisine keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq('-' * 60)
        expect(lines[12]).to include('Pepitos')
        expect(lines[13]).to eq('-' * 60)
      end
      ...
      it 'finds restaurants with prices less than keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq('-' * 60)
        expect(lines[12]).to include('Nandos')
        expect(lines[13]).to include('Samba Grill')
        expect(lines[14]).to include('Cafe Nero')
